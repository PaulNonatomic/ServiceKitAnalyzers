...
";
        ...
        private static Task Verify(string body) =>
            new CSharpAnalyzerTest<PreferExecuteWithCancellationAnalyzer, DefaultVerifier>
            {
                TestCode = Consumer(body),
            }.RunAsync();

        [TestMethod]
        public Task Inject_WithCancellationThenExecute_ReportsSK010() =>
            Verify(@"await _locator.Inject(this).WithCancellation(destroyCancellationToken).{|SK010:ExecuteAsync|}();");
        ...
        [TestMethod]
        public Task InjectServicesAsync_WithCancellationThenExecute_StillReportsSK010() =>
            Verify(@"await _locator.InjectServicesAsync(this).WithCancellation(destroyCancellationToken).{|SK010:ExecuteAsync|}();");
    }
}
